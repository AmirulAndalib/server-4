...
class SyncGroupPlayer(Player):
    ...
    def can_group_with(self) -> set[str]:
        ...
        # If we already have a sync leader, its grouping capabilities are the
        # source of truth for what can be joined immediately.
        if self.sync_leader:
            return {
                self.sync_leader.player_id,
                *self.sync_leader.state.can_group_with.difference(members_filter),
            }
        # If we have no sync leader, fall back to the first available member.
        for member_id in self._attr_group_members:
