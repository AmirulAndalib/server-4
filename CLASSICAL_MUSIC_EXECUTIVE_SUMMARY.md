...
I have been working with Claude and the full design is in [`CLASSICAL_MUSIC_MODEL_SPEC.md`](CLASSICAL_MUSIC_MODEL_SPEC.md) (~900 lines, all decisions captured).
...
What classical listeners actually want, synthesised from the MA "Better Classical Music Support" and other Discord threads, Roon's classical forums discussions, and Apple Music Classical's design choices:
...
UI mockups have been floated on Discord to gauge feedback, which has been very positive.
...
To keep it manageable the work will be split into 10 stages, each independently deployable. Each stage produces a reviewable PR; later stages depend on earlier ones for data shape but not for shipping behaviour.
